...
export default async function HandPage({ params: { lang } }: { params: { lang: Locale } }) {
  ...
  dict.nav.menu.forEach((menu) => {
    if (menu.href === 'horizontal-bar-graph-maker') {
      return;
    } else {
      result.push(menu);
    }
  ...
  const post = await getPost('horizontal-bar-graph-maker', lang) as unknown as { 
    title: string; 
    desc: string;
    contentHtml: string; 
  }
  ...
  return (
    ...
    <iframe src={lang === 'en' ? '/html/Horizontal-Bar-Graph-Maker.html' : '/html/Horizontal-Bar-Graph-Maker-CN.html'} allow="autoplay"  style={{ top: '0px', left: '0px',width: '100%', height: '1300px'}}></iframe>
...
export async function generateMetadata({ 
  ...
  return {
    ...
    alternates: {
      canonical: `${url}/${lang}/horizontal-bar-graph-maker`,
      languages: {
        'en': `${url}/en/horizontal-bar-graph-maker`,
        'zh': `${url}/zh/horizontal-bar-graph-maker`,
      },
    },
    openGraph: {
      title: post.title,
      description: post.desc,
      url: `${url}/${lang}/horizontal-bar-graph-maker`,
    }
  }
}
