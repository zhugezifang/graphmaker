...
export default async function HandPage({ params: { lang } }: { params: { lang: Locale } }) {
  ...
  dict.nav.menu.forEach((menu) => {
    if (menu.href === 'scatter-graph-maker') {
      return;
    } else {
      result.push(menu);
    }
  ...
  const post = await getPost('scatter-graph-maker', lang) as unknown as { 
    title: string; 
    desc: string;
    contentHtml: string; 
  }
  ...
  return (
    ...
    <iframe src={lang === 'en' ? '/html/Scatter-Graph-Maker.html' : '/html/Scatter-Graph-Maker-CN.html'} allow="autoplay"  style={{ top: '0px', left: '0px',width: '100%', height: '1250px'}}></iframe>
...
export async function generateMetadata({ 
  ...
  return {
    ...
    alternates: {
      canonical: `${url}/${lang}/scatter-graph-maker`,
      languages: {
        'en': `${url}/en/scatter-graph-maker`,
        'zh': `${url}/zh/scatter-graph-maker`,
      },
    },
    openGraph: {
      title: post.title,
      description: post.desc,
      url: `${url}/${lang}/scatter-graph-maker`,
    }
  }
}
