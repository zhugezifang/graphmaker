...
export default async function HandPage({ params: { lang } }: { params: { lang: Locale } }) {
  ...
  dict.nav.menu.forEach((menu) => {
    if (menu.href === 'column-chart-maker') {
      return;
    } else {
      result.push(menu);
    }
  ...
  const post = await getPost('column-chart-maker', lang) as unknown as { 
    title: string; 
    desc: string;
    contentHtml: string; 
  }
  ...
  return (
    ...
    <iframe src="/html/Column-Chart-Maker.html" allow="autoplay"  style={{ top: '0px', left: '0px',width: '100%', height: '1250px'}}></iframe>
...
export async function generateMetadata({ 
  ...
  return {
    ...
    alternates: {
      canonical: `${url}/${lang}/column-chart-maker`,
      languages: {
        'en': `${url}/en/column-chart-maker`,
        'zh': `${url}/zh/column-chart-maker`,
      },
    },
    openGraph: {
      title: post.title,
      description: post.desc,
      url: `${url}/${lang}/column-chart-maker`,
    }
  }
}
