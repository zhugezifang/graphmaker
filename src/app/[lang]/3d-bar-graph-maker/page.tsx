...
export default async function HandPage({ params: { lang } }: { params: { lang: Locale } }) {
  ...
  dict.nav.menu.forEach((menu) => {
    if (menu.href === '3d-bar-graph-maker') {
      return;
    } else {
      result.push(menu);
    }
  ...
  const post = await getPost('3d-bar-graph-maker', lang) as unknown as { 
    title: string; 
    desc: string;
    contentHtml: string; 
  }
  ...
  return (
    ...
    <iframe src={lang === 'en' ? '/html/3D-Bar-Chart-Maker.html' : '/html/3D-Bar-Chart-Maker-CN.html'} allow="autoplay"  style={{ top: '0px', left: '0px',width: '100%', height: '1200px'}}></iframe>
...
export async function generateMetadata({ 
  ...
  return {
    ...
    alternates: {
      canonical: `${url}/${lang}/3d-bar-graph-maker`,
      languages: {
        'en': `${url}/en/3d-bar-graph-maker`,
        'zh': `${url}/zh/3d-bar-graph-maker`,
      },
    },
    openGraph: {
      title: post.title,
      description: post.desc,
      url: `${url}/${lang}/3d-bar-graph-maker`,
    }
  }
}
