...
export default async function HandPage({ params: { lang } }: { params: { lang: Locale } }) {
  ...
  dict.nav.menu.forEach((menu) => {
    if (menu.href === 'radar-graph-maker') {
      return;
    } else {
      result.push(menu);
    }
  ...
  const post = await getPost('radar-graph-maker', lang) as unknown as { 
    title: string; 
    desc: string;
    contentHtml: string; 
  }
  ...
  return (
    ...
    <iframe src="/html/Radar-Graph-Maker.html" allow="autoplay"  style={{ top: '0px', left: '0px',width: '100%', height: '1610px'}}></iframe>
...
export async function generateMetadata({ 
  ...
  return {
    ...
    alternates: {
      canonical: `${url}/${lang}/radar-graph-maker`,
      languages: {
        'en': `${url}/en/radar-graph-maker`,
        'zh': `${url}/zh/radar-graph-maker`,
      },
    },
    openGraph: {
      title: post.title,
      description: post.desc,
      url: `${url}/${lang}/radar-graph-maker`,
    }
  }
}
