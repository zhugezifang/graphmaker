...
export default async function HandPage({ params: { lang } }: { params: { lang: Locale } }) {
  ...
  dict.nav.menu.forEach((menu) => {
    if (menu.href === 'break-even-chart-maker') {
      return;
    } else {
      result.push(menu);
    }
  ...
  const post = await getPost('break-even-chart-maker', lang) as unknown as { 
    title: string; 
    desc: string;
    contentHtml: string; 
  }
  ...
  return (
    ...
    <iframe src="/html/break-even-chart-maker.html" allow="autoplay"  style={{ top: '0px', left: '0px',width: '100%', height: '1200px'}}></iframe>
...
export async function generateMetadata({ 
  ...
  return {
    ...
    alternates: {
      canonical: `${url}/${lang}/break-even-chart-maker`,
      languages: {
        'en': `${url}/en/break-even-chart-maker`,
        'zh': `${url}/zh/break-even-chart-maker`,
      },
    },
    openGraph: {
      title: post.title,
      description: post.desc,
      url: `${url}/${lang}/break-even-chart-maker`,
    }
  }
}
