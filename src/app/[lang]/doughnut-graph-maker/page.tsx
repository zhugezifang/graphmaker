...
export default async function HandPage({ params: { lang } }: { params: { lang: Locale } }) {
  ...
  dict.nav.menu.forEach((menu) => {
    if (menu.href === 'doughnut-graph-maker') {
      return;
    } else {
      result.push(menu);
    }
  ...
  const post = await getPost('doughnut-graph-maker', lang) as unknown as { 
    title: string; 
    desc: string;
    contentHtml: string; 
  }
  ...
  return (
    ...
    <iframe src={lang === 'en' ? '/html/Doughnut-Chart-Maker.html' : '/html/Doughnut-Chart-Maker-CN.html'} allow="autoplay"  style={{ top: '0px', left: '0px',width: '100%', height: '1200px'}}></iframe>
...
export async function generateMetadata({ 
  ...
  return {
    ...
    alternates: {
      canonical: `${url}/${lang}/doughnut-graph-maker`,
      languages: {
        'en': `${url}/en/doughnut-graph-maker`,
        'zh': `${url}/zh/doughnut-graph-maker`,
      },
    },
    openGraph: {
      title: post.title,
      description: post.desc,
      url: `${url}/${lang}/doughnut-graph-maker`,
    }
  }
}
